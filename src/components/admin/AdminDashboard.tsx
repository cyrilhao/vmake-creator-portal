      ...
      <div className="flex min-h-screen">
        ...
        <section className="min-w-0 flex-1">
          ...
          <div className="space-y-6 px-4 py-6 md:px-8">
              ...
              <div className="rounded-lg border border-white/10 bg-white/[0.04]">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 px-4 py-4">
                  <div>
                    ...
                    <p className="text-sm text-slate-400">May 2026 creator submissions</p>
                  </div>
            ...
            <section className="rounded-lg border border-white/10 bg-white/[0.04] p-5" id="audit-trail">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h3 className="text-lg font-semibold">Audit trail</h3>
                  <p className="text-sm text-slate-400">
                    Every manual override is stamped with reason and operator.
                  </p>
                </div>
                <span className="rounded-full border border-white/10 px-3 py-1 text-xs font-semibold text-slate-300">
                  27 changes this month
                </span>
              </div>

              <div className="mt-5 grid gap-3">
                {[
                  ["16:48", "Elaine Wu", "Adjusted final reward to $120", "Reason: flagged YouTube proof pending"],
                  ["14:05", "Marcus Lin", "Marked payout approved", "Batch: May creator payouts"],
                  ["09:22", "Elaine Wu", "Rejected duplicate submission", "Creator already submitted May cycle"],
                ].map(([time, admin, action, note]) => (
                  <div
                    className="flex flex-col gap-2 rounded-lg border border-white/10 bg-[#0b1020] p-4 md:flex-row md:items-start md:justify-between"
                    key={`${time}-${admin}-${action}`}
                  >
                    <div>
